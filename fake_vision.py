...
def run_vision():
    ...
    for idx, t in enumerate(robot_hand):
        robot_hand_poses[f"{t[0]}_{t[1]}"] = {"x": 0.2 + (idx * 0.05), "y": 0.3, "theta": 0.0}

    # Asignamos una posición inicial a la primera ficha del tablero (Centro de la mesa)
    board_poses = {f"{board[0][0]}_{board[0][1]}": {"x": 0.5, "y": 0.5, "theta": 0.0}}
    ...
    while True:
                ...
                if board:
                    ...
                    for t in human_hand:
                        if t[0] == l_val or t[1] == l_val:
                            ...
                            board_poses[f"{tile[0]}_{tile[1]}"] = {"x": 0.5, "y": 0.5 + (len(board)*0.05), "theta": 0.0}
                            played = True
                            break
        ...
        if socket_control in socks and socks[socket_control] == zmq.POLLIN:
            ...
            if accion == "MOVE":
                ...
                board_poses[f"{tile[0]}_{tile[1]}"] = data.get("place_pose", {"x": 0.5, "y": 0.5, "theta": 0.0})
                ...
                if all_dominoes:
                    ...
                    # Simulamos que la nueva ficha robada tiene una posición física aleatoria
                    robot_hand_poses[f"{nueva[0]}_{nueva[1]}"] = {"x": random.uniform(0.1, 0.4), "y": random.uniform(0.4, 0.6), "theta": random.uniform(-90, 90)}
