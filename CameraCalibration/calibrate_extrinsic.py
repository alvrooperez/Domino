...
def to_display(frame):
    """Redimensiona a 800×450 (16:9, igual que el detector) sin modificar el frame original."""
    h, w = frame.shape[:2]
    scale_u = 800 / w
    scale_v = 450 / h
    return cv2.resize(frame, (800, 450)), scale_u, scale_v
...
def _mouse_cb(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        _clicked = (int(x / _scale_u), int(y / _scale_v))


# ── Main ──────────────────────────────────────────────────────────────────────
def main():
    global _clicked, _scale_u, _scale_v
    ...
    # Imprimir resolución real para confirmar
    ret, test = cap.read()
    if ret:
        h, w = test.shape[:2]
    ...
    while True:
        ...
        disp_small, _scale_u, _scale_v = to_display(display)
